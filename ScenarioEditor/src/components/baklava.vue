...
import WarningTextOption from "./options/WarningTextOption.vue";
...
export default {
  ...
  methods: {
    ...
    updateSettings() {
      ...
      for (const node of nodes) {
        if (node.name === "Start") {
          node.setOptionValue("Settings", node.updateSettings());
          const option = node.options.get("Settings");
          option.events.updated.emit();
        }
      }
    },
  ...
  created() {
    ...
    this.viewPlugin.registerOption("WarningTextOption", WarningTextOption);
  ...
  watch: {
    ...
    "$store.state.showIfDecisionWasCorrect": function () {
      this.updateStartNode();
    },
    "$store.state.allowIncorrectDecision": function () {
      this.updateStartNode();
    },
    "$store.state.useTTS": function () {
      this.updateStartNode();
    },
  },
};
